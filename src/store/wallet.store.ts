...
export const useWalletStore = create<WalletState>((set, get) => {
    ...
    return {
        ...
        unlock: async (walletId: string, password: string) => {
            try {
                const wallet = await _walletStorage.getDecrypted(
                    walletId,
                    password
                )
                ...
                _accountService.on('transactionReceived', async (txDetails) => {
                    if (txDetails.payload?.startsWith('636970685f6d73673a')) {
                        ...
                        const recipientAddress =
                            messageOutput?.script_public_key_address ||
                            'Unknown'
                        ...
                        const myAddress =
                            _accountService?.receiveAddress?.toString() || ''
                ...
                if (initialBalance) {
                    ...
                    set({
                        balance: {
                            ...
                            matureDisplay: sompiToKaspaString(
                                initialBalance.mature
                            ),
                            pendingDisplay: sompiToKaspaString(
                                initialBalance.pending
                            ),
                            outgoingDisplay: sompiToKaspaString(
        ...
        estimateSendMessageFees: async (
            ...
            if (!state.unlockedWallet || !state.accountService) {
                throw new Error(
                    'Wallet not unlocked or account service not running'
                )
            }
