...
export const useMessagingStore = create<MessagingState>((set, g) => ({
        ...
        if (walletAddress) {
            messages.forEach((message) => {
                ...
                if (existingContactIndex !== -1) {
                    ...
                    if (
                        message.timestamp >
                        existingContact.lastMessage.timestamp
                    ) {
    ...
    storeMessage: (message: Message, walletAddress: string) => {
            ...
            try {
                ...
                if (
                    ...
                    (handshakePayload.isResponse &&
                        message.senderAddress ===
                            handshakePayload.recipientAddress)
                ) {
                    ...
                    g()
                        ...
                        .catch((error) => {
                            if (
                                ...
                                console.log(
                                    'Skipping self-conversation handshake'
                                )
                                return
                            }
        ...
        if (existingMessageIndex !== -1) {
            ...
            const existingMessage =
                messagesMap[walletAddress][existingMessageIndex]
            const mergedMessage = {
                ...message,
                ...
                senderAddress:
                    message.senderAddress || existingMessage.senderAddress,
                recipientAddress:
                    message.recipientAddress ||
                    existingMessage.recipientAddress,
            }
        ...
        if (existingContactIndex !== -1) {
            ...
            const updatedContact = {
                ...
                lastMessage: message,
                messages: [
                    ...state.contacts[existingContactIndex].messages,
                    message,
                ],
            }
    ...
    exportMessages: async (wallet, password) => {
        try {
            ...
            const backup = {
                ...
                conversations: {
                    active:
                        g().conversationManager?.getActiveConversations() || [],
                    pending:
                        g().conversationManager?.getPendingConversations() ||
                        [],
                },
            }
            ...
            if (decryptedData.conversations) {
                ...
                const isValidConversation = (
                    ...
                    return (
                        ...
                        (conv.theirAlias === null ||
                            typeof conv.theirAlias === 'string') &&
                ...
                active.forEach((conv: ActiveConversation) => {
                    if (isValidConversation(conv)) {
                        ...
                        console.error(
                            'Invalid conversation object in backup:',
                            conv
                        )
                    }
                })
    ...
    respondToHandshake: async (handshake: HandshakeState) => {
        try {
            ...
            console.log(
                'Sending handshake response to:',
                handshake.kaspaAddress
            )
            ...
            try {
                ...
                console.log(
                    'Valid Kaspa address created:',
                    kaspaAddress.toString()
                )
                ...
                const conversation =
                    manager.getConversationByAddress(recipientAddress)
                if (conversation) {
