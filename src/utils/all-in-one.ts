...
export async function fetchTransactionDetails(
    ...
    try {
        ...
        if (
            !txData.accepting_block_hash ||
            !txData.accepting_block_blue_score
        ) {
...
export class KaspaClient {
    ...
    constructor(args?: KaspaClientArgs) {
        ...
        console.log(
            `KaspaClient initialized with network ID: ${this.networkId}`
        )
    }
            ...
            try {
                ...
                await Promise.race([
                    ...
                    new Promise((_, reject) => {
                        console.log(
                            'Setting resolver connection timeout for 20s'
                        )
                        setTimeout(
                ...
                if (this.rpc && this.rpc.isConnected) {
                    ...
                    throw new Error(
                        'Connection lost after initial establishment'
                    )
                }
            } catch (error) {
                console.log(
    ...
    async subscribeToUtxoChanges(
        ...
        const attemptSubscribe = async (): Promise<void> => {
            try {
                ...
                console.log(
                    `Subscribing to UTXO changes for addresses: ${addresses}`
                )
                ...
                const wrappedWithFilter = (notification: IUtxosChanged) => {
                    ...
                    if (notEmittedTxIds.length > 0) {
                        ...
                        this.historyOfEmittedTxIdUtxoChanges.push(
                            ...notEmittedTxIds
                        )
