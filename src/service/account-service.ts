...
export class AccountService extends EventEmitter<AccountServiceEvents> {
    ...
    async start() {
        try {
            ...
            this.receiveAddress = this.ensureAddressPrefix(
                initialReceiveAddress
            )
    ...
    async stop() {
        ...
        try {
            ...
            console.log(
                'Successfully cleaned up UTXO subscription and processor'
            )
        } catch (error) {
            console.error(
    ...
    public async createTransaction(
        ...
        try {
            ...
            const privateKeys = pendingTransaction.addresses().map(() => {
                ...
                if (!key) {
                    throw new Error(
                        'Failed to generate private key for signing'
                    )
                }
                return key
            })
            ...
            const txId: string = await pendingTransaction.submit(
                this.rpcClient.rpc
            )
    ...
    private async loadConversations(): Promise<void> {
        try {
            ...
            if (!this.conversationsLoaded) {
                console.warn(
                    'Could not load conversations after maximum retries'
                )
    ...
    public async sendMessage(
        ...
        const destinationAddress = this.ensureAddressPrefix(
    ...
    private adjustForSEC1Format(encryptedHex: string): string {
        ...
        console.log(
            'Detected SEC1 compressed key format in pre-encrypted message'
        )
    ...
    private _getGeneratorForTransaction(transaction: CreateTransactionArgs) {
        ...
        console.log(
            'Using primary address for change:',
            primaryAddress.toString()
        )
        ...
        if (conversationManager) {
            const conversations =
                conversationManager.getMonitoredConversations()
        ...
        const isSelfMessage =
            ...
            (isDirectSelfMessage || hasActiveConversation)
    ...
    private _getGeneratorForWithdrawTransaction(
        ...
        const isFullBalance =
            transaction.amount === this.context.balance?.mature
        ...
        return new Generator({
            ...
            outputs: [
                new PaymentOutput(transaction.address, transaction.amount),
            ],
            networkId: this.networkId,
            priorityFee: {
                amount: BigInt(0),
        ...
        try {
            ...
            if (tx.inputs && tx.inputs.length > 0) {
                ...
                if (prevTxId && typeof prevOutputIndex === 'number') {
                    try {
                        const prevTx =
                            await this._fetchTransactionDetails(prevTxId)
                        if (
                            prevTx?.outputs &&
                            prevTx.outputs[prevOutputIndex]
                        ) {
            ...
            const isMonitoredAddress =
                ...
                (recipientAddress &&
                    this.monitoredAddresses.has(recipientAddress))
            const isCommForUs =
                messageType === 'comm' &&
                targetAlias &&
            ...
            try {
                ...
                try {
                    ...
                    if (process.env.NODE_ENV === 'development') {
                        console.debug(
                            `Failed to decrypt with receive key:`,
                            error
                        )
                    }
                }
                ...
                if (!decryptionSuccess) {
                    try {
                        ...
                        if (process.env.NODE_ENV === 'development') {
                            console.debug(
                                `Failed to decrypt with change key:`,
                                error
                            )
                        }
                    }
    ...
    private isTransactionForUs(tx: ITransaction): boolean {
        ...
        const messageAddress = messageOutput
            ...
            : null
        const changeAddress = changeOutput
            ? getOutputAddress(changeOutput)
            : null
    ...
    public async sendMessageWithContext(
        sendMessage: SendMessageWithContextArgs
    ) {
        this.ensurePasswordSet()
    ...
    private async updateMonitoredConversations() {
        try {
            const { useMessagingStore } = await import(
            ...
            conversations.forEach(
                (conv: { alias: string; address: string }) => {
            ...
            for (const tx of transactions) {
                ...
                if (this.isMessageTransaction(tx)) {
                    ...
                    if (
                        ...
                        const oldestId = this.processedMessageIds
                            .values()
                            .next().value
                    ...
                    try {
                        ...
                        if (process.env.NODE_ENV === 'development') {
                            console.debug(
                                'Error processing message transaction:',
                                error
                            )
                        }
                    }
